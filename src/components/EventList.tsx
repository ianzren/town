...
export function EventList({ initialEvents = [] }: EventListProps) {
  ...
  const todayEvents = events.filter(
    (e) => e.showTime >= todayStart && e.showTime < todayEnd,
  );
  ...
  return (
    <div className="w-full">
      ...
      {futureEvents.length > 0 && (
        <>
          <h2 className="text-xl font-normal text-zinc-400 mb-2.5 mt-10">
            Upcoming
          </h2>
...
function EventCard({ event, compact = false }: EventCardProps) {
  ...
  return (
    <div className="flex flex-row gap-2.5 items-stretch">
      ...
      >
        <h3
          className={`font-bold m-0 -mt-1.5 ${compact ? "text-sm" : "text-base"}`}
        >
          {event.artist}
        </h3>
        {event.opener && (
        ...
        {event.tour && (
          <p className="text-[0.8em] text-zinc-400 m-0 mt-0.5">{event.tour}</p>
        )}
      </a>

      <div
        className={`flex-1 flex flex-col text-zinc-400 p-2 ${compact ? "text-xs" : "text-sm"}`}
      >
