...
function extractEvents(
  ...
  for (const entry of dom.window.document.querySelectorAll(".entry")) {
    ...
    const tour =
      entry.querySelector(".accentColor + h5")?.textContent?.trim() || null;
    const artist = entry
      .querySelector(".carousel_item_title_small")
      ?.textContent?.trim();
    const url = entry
      .querySelector("h3 a")
    ...
    const opener =
      entry
        .querySelector(".carousel_item_title_small + h4")
        ?.textContent?.trim()
        .replace(/^with\s+/i, "") || null;
    const showDateStr = entry.querySelector(".date")?.textContent?.trim();
    const showTimeStr = entry
      .querySelector(".time")
      ?.textContent?.trim()
      .replace(/^show\s+/i, "");
...
export async function fetchAxsEvents(
  baseUrl: string,
  venue: string,
  timeZone: string,
): Promise<VenueEvent[]> {
